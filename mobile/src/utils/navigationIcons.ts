...
const RAW_ICON_MAP: Record<string, Component> = {
  users: UsersIcon,
  user: UserGroupIcon,
  ...
  home: HomeIcon,
  ...Object.fromEntries(
    Object.entries(MODULE_ICON_IDS).map(([moduleKey]) => [
      moduleKey,
      MODULE_ICON_COMPONENTS[moduleKey]
    ])
  )
}
...
export const SHELL_TAB_ICONS: Record<string, Component> = {
  home: HomeIcon,
  inbox: InboxIcon,
  tasks: CheckCircleIcon,
  apps: Squares2X2Icon
...
export function getIconComponent(icon?: string, moduleKey?: string): Component {
...
export function getNavigationIconComponent(item: IconLookupItem): Component {
