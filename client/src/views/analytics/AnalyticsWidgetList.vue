    ...
    <div v-if="showTemplates" class="px-4 py-6 sm:px-6">
      <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <AnalyticsListHeaderActions
          active="widgets"
          :create-label="t('analytics.newWidget')"
          show-list
          @create="goCreate"
    ...
    >
      ...
      <template #cell-chartType="{ value }">
        <span class="capitalize text-gray-700 dark:text-gray-300">
          {{ t(`analytics.chartType_${value}`, value) }}
        </span>
      </template>

      <template #cell-reportId="{ row }">
        <span class="text-gray-700 dark:text-gray-300">{{ reportName(row) }}</span>
      </template>
