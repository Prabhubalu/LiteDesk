    ...
    <div class="grid gap-6 lg:grid-cols-[minmax(0,320px)_1fr]">
      <section class="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
        ...
        <form class="mt-4 space-y-3" @submit.prevent="handleSubmit">
          ...
          <div>
            <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">{{ t('contentStudio.categoryEmojiLabel') }}</label>
            <input v-model="formEmoji" type="text" maxlength="8" :class="inputClass" :placeholder="t('contentStudio.collectionEmojiPlaceholder')" />
          </div>
          <div>
...
function startEdit(category) {
  ...
  formEmoji.value = category.emoji || '';
...
async function handleSubmit() {
  ...
  try {
    const payload = {
      name,
      emoji: formEmoji.value.trim() || undefined,
