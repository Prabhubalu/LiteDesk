<template>
  <div>
    <InventoryTxnList
      title="Sales Returns"
      subtitle="Return invoiced goods to stock"
      :columns="columns"
      :rows="rows"
      :loading="loading"
      :error="error"
      create-label="New Return"
      empty-label="No sales returns yet."
    ...
    <Teleport to="body">
      <div v-if="showCreate" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" @click.self="showCreate = false">
        <form class="w-full max-w-lg space-y-3 rounded-xl bg-white p-5 shadow-xl dark:bg-gray-800" @submit.prevent="create">
          <h3 class="text-base font-semibold text-gray-900 dark:text-white">New Sales Return</h3>
          <label class="block text-sm">
            <span>Invoice ID</span>
            <input v-model="form.invoiceId" required class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white" />
          </label>
          <label class="block text-sm">
          ...
          <label class="block text-sm">
            ...
            <input v-model="form.overallReturnReason" required class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white" />
          </label>
...
const form = reactive({ invoiceId: '', returnLocationId: '', overallReturnReason: 'Customer return' });
const columns = [
  { key: 'salesReturnNumber', label: 'SR #' },
...
async function load() {
  loading.value = true;
  try {
    const [sr, locs] = await Promise.all([
      apiClient.get('/inventory/sales-returns'),
      apiClient.get('/inventory/locations')
    ]);
    rows.value = unwrapList(sr);
...
async function create() {
  ...
  formError.value = '';
  try {
    const detail = await apiClient.get(`/invoices/${form.invoiceId}`);
    const inv = detail?.data || detail;
    const invLines = Array.isArray(inv?.lines) ? inv.lines : Array.isArray(detail?.data?.lines) ? detail.data.lines : [];
    // Fallback: list lines endpoint if header payload has none
    let lines = invLines;
    if (!lines.length) {
      const lineRes = await apiClient.get(`/invoices/${form.invoiceId}/lines`).catch(() => null);
      lines = unwrapList(lineRes);
    }
    if (!lines.length) throw new Error('Invoice has no lines');
    await apiClient.post('/inventory/sales-returns', {
      invoiceId: form.invoiceId,
      returnLocationId: form.returnLocationId,
      overallReturnReason: form.overallReturnReason,
      lines: lines
        .map((l) => {
          const available = Number(l.quantity || 0) - Number(l.quantityReturned || 0);
          return available > 0
            ? {
                invoiceLineId: l._id || l.invoiceLineId,
...
async function approve(row) {
  try {
    await apiClient.post(`/inventory/sales-returns/${row._id}/approve`);
    await load();
  } catch (err) {
