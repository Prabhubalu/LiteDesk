<template>
  ...
  >
    <div v-if="showSetup" class="mx-auto max-w-lg">
      <div class="hes-card">
        <h2 class="text-lg font-semibold text-neutral-900">{{ t('contentStudio.headlessCategoryExampleTitle') }}</h2>
        <p class="mt-1 text-sm text-neutral-500">{{ t('contentStudio.headlessCategoryExampleDesc') }}</p>
        <p class="mt-4 text-sm text-neutral-700">{{ t('contentStudio.headlessCategoryExampleSetup') }}</p>
        <form class="mt-4 space-y-4" @submit.prevent="submitSetup">
          ...
          <label class="block text-sm font-medium text-neutral-700">
            {{ t('contentStudio.headlessCategoryExampleCollectionLabel') }}
            <input
              v-model="setupCollection"
              type="text"
              required
              class="mt-1 w-full rounded-lg border border-neutral-300 bg-white px-3 py-2 text-sm"
            />
    ...
    <template v-else>
      ...
      <div id="ld-help-category-example">
...
const error = ref('');
const setupOrg = ref('');
const setupCollection = ref('');

const orgSlug = computed(() => String(route.query.org || route.query.orgSlug || '').trim());
const collectionSlug = computed(() => String(route.query.collection || '').trim());
const showSetup = computed(() => !orgSlug.value || !collectionSlug.value);
...
async function loadEmbedScripts() {
  ...
  await loadScript('/embed/headless-help-category.js', 'headless-help-category-js');
}
...
async function loadCategory() {
  if (showSetup.value) {
    ...
    setupCollection.value = collectionSlug.value;
    loading.value = false;
    return;
  }
  ...
  try {
    ...
    await window.ArivuHeadlessHelpCategory.mount({
      ...
      apiOrigin,
      linkPrefix: prefixes.categoryPrefix,
      ...
      sectionsEmptyLabel: t('contentStudio.publicHelpSectionsEmpty'),
      notFoundLabel: t('contentStudio.publicHelpCategoryNotFound'),
    ...
    trackHeadlessHelpViewed('category', { org: orgSlug.value, collection: collectionSlug.value });
  } catch (err) {
    error.value = err?.message || t('contentStudio.headlessCategoryExampleLoadFailed');
  } finally {
    loading.value = false;
  }
}
...
function submitSetup() {
  router.replace({
    ...
    query: {
      ...
      collection: setupCollection.value.trim(),
    },
  });
}
...
watch(
  () => [route.query.org, route.query.orgSlug, route.query.collection],
  () => {
    void loadCategory();
  },
);
