<template>
  <div class="mx-auto max-w-3xl space-y-4">
    <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 sm:p-6">
      <h2 class="text-base font-semibold text-gray-900 dark:text-white">Assistant</h2>
      <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
        Ask about sync health, queue, GST, duplicates, or mappings.
      </p>
      ...
      <div class="mt-4 max-h-80 space-y-3 overflow-auto rounded-xl border border-gray-100 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-900/40">
        <div v-if="!messages.length" class="py-8 text-center text-sm text-gray-500">No messages yet.</div>
        <div
...
async function ask() {
  ...
  try {
    ...
    error.value = err?.response?.data?.message || err?.message || 'Assistant failed';
