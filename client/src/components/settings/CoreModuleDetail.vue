<template>
  <SettingsScrollPanel
    :embed="usesModulesAndFields && !!module && !loading && !error"
    :dense="usesModulesAndFields && !!module && !loading && !error"
  >
    <template #header>
      <button
        @click="goBack"
      ...
      <div
        v-if="module && !loading && !error && usesModulesAndFields"
        class="mt-1.5 flex min-w-0 flex-wrap items-center gap-x-2 gap-y-1"
      >
        <div class="flex shrink-0 items-center justify-center w-8 h-8 rounded-lg bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4m0 5c0 2.21-3.582 4-8 4s-8-1.79-8-4" />
          </svg>
        </div>
        <h2 class="truncate text-gray-900 dark:text-white">
          {{ module.name }}
        </h2>
        <span
          v-if="module.platformOwned"
          class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300"
        >
        ...
        <span
          ...
          class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300"
        >
          <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div v-else-if="module" :class="usesModulesAndFields ? 'flex min-h-0 flex-1 flex-col overflow-hidden' : 'space-y-6'">
      ...
      <div v-else-if="isItemsModule || isQuotesModule || isSalesOrdersModule || isInvoicesModule || isPaymentsModule || isDocumentsModule" class="flex min-h-0 flex-1 flex-col overflow-hidden">
...
const catalogEntityModuleFilter = (module) => {
  ...
  if (isDocumentsModule.value) return documentsModuleFilter(module);
  return false;
};
...
const catalogEntityModuleTitle = computed(() => {
  ...
  if (isDocumentsModule.value) return t('settings.coreModDetailModuleDocuments');
  return '';
});
...
const documentsModuleFilter = (module) => {
  return module.key?.toLowerCase() === 'documents';
};
...
watch(() => moduleKey.value, (newKey) => {
  ...
  if ((upperKey === 'PEOPLE' || upperKey === 'ORGANIZATIONS' || upperKey === 'TASKS' || upperKey === 'EVENTS' || upperKey === 'FORMS' || upperKey === 'ITEMS' || upperKey === 'QUOTES' || upperKey === 'SALES_ORDERS' || upperKey === 'INVOICES' || upperKey === 'PAYMENTS' || upperKey === 'DOCUMENTS') && !route.query.module) {
