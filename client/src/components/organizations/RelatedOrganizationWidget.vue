<template>
  <CardWidget :key="refreshKey" :title="t('organizations.relatedOrganizationWidgetRelatedOrganizations')" class="ld-card-group">
    <template #actions>
      <button
        ...
        :title="t('organizations.relatedOrganizationWidgetLinkOrganization')"
      >
            ...
            <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
              <MenuItems class="absolute right-0 mt-2 w-40 rounded-lg shadow-xl py-1 bg-white dark:bg-gray-800 ring-1 ring-black/5 dark:ring-white/10 z-10">
                <MenuItem v-slot="{ active }">
                  <button @click="$emit('unlink-organization', organization._id)" :class="['w-full text-left px-4 py-2 text-sm', active ? 'bg-gray-100 dark:bg-gray-700' : '']">{{ t('records.relatedUnlink') }}</button>
                </MenuItem>
      ...
      <div v-else class="text-center py-8">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('organizations.relatedOrganizationWidgetNoOrganizationLinked') }}</p>
      </div>
  </CardWidget>
</template>
