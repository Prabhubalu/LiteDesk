    ...
    <Transition name="email-compose-drawer">
      ...
      >
        ...
        <aside
          ...
          role="dialog"
          aria-modal="true"
          :aria-label="t('records.drSendEmailTitle')"
        >
          <div
          ...
          <form class="flex min-h-0 flex-1 flex-col" @submit.prevent="submit">
            <div class="flex-1 space-y-4 overflow-y-auto p-6">
              ...
              <div>
                ...
                <input
                  ...
                  :placeholder="t('records.drSendEmailToPlaceholder')"
                  :disabled="sending"
                />
              </div>
              ...
              <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-200">
                ...
                {{ t('records.drSendEmailAttachPdf') }}
              </label>
            </div>
            ...
            >
              ...
              <button
                ...
                :disabled="sending"
              >
                {{ sending ? t('records.drSendEmailSending') : t('records.drSendEmailSubmit') }}
              </button>
            </div>
          </form>
        </aside>
...
function resetForm() {
  ...
  const number = props.record?.deliveryReturnNumber || 'DR';
  const title = props.record?.subject ? ` — ${props.record.subject}` : '';
  subject.value = t('records.drSendEmailDefaultSubject', { number, title: title || '' });
  message.value = t('records.drSendEmailDefaultMessage');
...
async function submit() {
  ...
  error.value = '';
  try {
    const res = await apiClient.post(`/inventory/delivery-returns/${props.record._id}/send-email`, {
      to: to.value.trim(),
    ...
    if (!res?.success) {
      throw new Error(res?.message || t('records.drSendEmailFailed'));
    }
    emit('sent', res.data);
    emit('close');
  } catch (e) {
    error.value = e?.message || t('records.drSendEmailFailed');
  } finally {
    sending.value = false;
  }
}
