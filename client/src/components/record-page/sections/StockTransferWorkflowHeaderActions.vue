...
function transferId() {
  return props.record?.inventoryTransferId || props.record?._id || props.record?.id || null;
}
...
async function post() {
  ...
  busy.value = true;
  try {
    const res = await apiClient.post(`/inventory/transfers/${id}/post`);
    const data = res?.data?.transfer ?? res?.data ?? res;
    if (typeof emitSectionUpdated === 'function') {
      emitSectionUpdated({ payload: data });
    }
    notify('success', t('records.xferPosted'));
  } catch (err) {
    notify('error', err?.message || t('records.xferActionFailed'));
  } finally {
    busy.value = false;
  }
}
...
const workflowActions = computed(() => {
  const list = [];
  if (status.value === 'draft' || status.value === 'in_transit') {
    list.push({
      key: 'post',
      label: t('records.xferPost'),
      className:
