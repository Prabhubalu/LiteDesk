    ...
    >
      ...
      >
        <div>
          ...
          >
            {{ t('records.salesOrderPartialSection') }}
          </div>
        </div>
        ...
        <table class="min-w-full text-sm">
          ...
          <tbody>
            <tr
              ...
              :key="line._id || line.salesOrderLineId"
            ...
            >
              <td class="px-3 py-2">
                ...
                >
                  {{ t('records.salesOrderBundleParent') }}
                </div>
              </td>
              <td class="px-3 py-2 text-right tabular-nums">
                <input
                  v-if="isLineEditable(line)"
                  v-model.number="draftQty[line.salesOrderLineId]"
                  type="number"
                  min="1"
              ...
              <td v-if="isDraftEditable" class="px-3 py-2 text-right">
                <div v-if="isLineEditable(line)" class="flex flex-col items-end gap-1">
                  <select
                    ...
                    :value="line.salesOrderSectionId ? String(line.salesOrderSectionId) : ''"
                  ...
                  <button
                    ...
                    :disabled="busyLineId === line.salesOrderLineId"
                    @click="removeLine(line)"
                  >
                    {{ t('records.linesRemove') }}
                  </button>
                </div>
                <span v-else class="text-xs text-gray-400">{{ t('records.salesOrderBundleComponentHint') }}</span>
              </td>
            </tr>
          </tbody>
        </table>
...
watch(
  ...
  (lines) => {
    for (const line of lines || []) {
      ...
      draftQty[line.salesOrderLineId] = Number(line.quantity) || 1;
    }
  },
...
function applyMutationToRecord({ line, lines, totals, sections, deletedLine }) {
  ...
  if (deletedLine && Array.isArray(props.record?.lines)) {
    ...
    props.record.lines = props.record.lines.filter((row) => {
      if (String(row.salesOrderLineId) === String(deletedLine.salesOrderLineId)) return false;
...
async function saveQuantity(line) {
  ...
  busyLineId.value = line.salesOrderLineId;
  try {
    const res = await apiClient.patch(
      `/sales-orders/${props.record._id}/lines/${line.salesOrderLineId}`,
      { quantity: nextQty }
    );
    if (!res?.success) {
      notifications.error(res?.message || t('records.salesOrderLineUpdateFailed'));
      draftQty[line.salesOrderLineId] = Number(line.quantity) || 1;
      return;
    }
    ...
    notifications.success(t('records.salesOrderLineUpdated'));
  } catch (e) {
    notifications.error(e?.message || t('records.salesOrderLineUpdateFailed'));
    draftQty[line.salesOrderLineId] = Number(line.quantity) || 1;
  } finally {
    busyLineId.value = '';
  }
...
async function moveToSection(line, sectionId) {
  if (!sectionId || String(line.salesOrderSectionId || '') === String(sectionId)) return;
  busyLineId.value = line.salesOrderLineId;
  try {
    const res = await apiClient.patch(
      `/sales-orders/${props.record._id}/lines/${line.salesOrderLineId}`,
    ...
    if (!res?.success) {
      notifications.error(res?.message || t('records.salesOrderLineMoveFailed'));
      return;
    }
    emitSectionUpdate({ type: 'soft-refresh' });
    notifications.success(t('records.salesOrderLineMoved'));
  } catch (e) {
    notifications.error(e?.message || t('records.salesOrderLineMoveFailed'));
  } finally {
    busyLineId.value = '';
  }
}
...
async function removeLine(line) {
  ...
  try {
    ...
    if (!res?.success) {
      notifications.error(res?.message || t('records.salesOrderLineDeleteFailed'));
      return;
    }
    applyMutationToRecord(res.data);
    emitSectionUpdate({ type: 'line-deleted', ...res.data });
    notifications.success(t('records.salesOrderLineDeleted'));
  } catch (e) {
    notifications.error(e?.message || t('records.salesOrderLineDeleteFailed'));
  } finally {
    busyLineId.value = '';
  }
}
...
function openEditSection(section) {
  ...
  sectionModalInitial.value = {
    ...
    includeInQuoteTotal: section.includeInOrderTotal !== false
  };
  sectionModalEditingId.value = section.salesOrderSectionId || section._id;
  sectionModalOpen.value = true;
}
...
async function submitSectionModal(form) {
  ...
  try {
    const body = {
      ...
      includeInOrderTotal: form.includeInQuoteTotal
    };
    const res =
      sectionModalMode.value === 'edit' && sectionModalEditingId.value
        ? await apiClient.patch(
            `/sales-orders/${props.record._id}/sections/${sectionModalEditingId.value}`,
            body
          )
        : await apiClient.post(`/sales-orders/${props.record._id}/sections`, body);
    if (!res?.success) {
      notifications.error(res?.message || t('records.salesOrderSectionSaveFailed'));
      return;
    }
    applySectionMutation(res.data);
    notifications.success(
      sectionModalMode.value === 'edit'
        ? t('records.salesOrderSectionUpdated')
        : t('records.salesOrderSectionCreated')
    );
    closeSectionModal();
  } catch (e) {
    notifications.error(e?.message || t('records.salesOrderSectionSaveFailed'));
  } finally {
...
async function deleteSection(section) {
  if (!window.confirm(t('records.salesOrderSectionDeleteConfirm'))) return;
  const sectionId = section.salesOrderSectionId || section._id;
  sectionBusy.value = true;
  try {
    const res = await apiClient.delete(`/sales-orders/${props.record._id}/sections/${sectionId}`);
    if (!res?.success) {
      notifications.error(res?.message || t('records.salesOrderSectionDeleteFailed'));
      return;
    }
    applySectionMutation(res.data);
    notifications.success(t('records.salesOrderSectionDeleted'));
  } catch (e) {
    notifications.error(e?.message || t('records.salesOrderSectionDeleteFailed'));
  } finally {
    sectionBusy.value = false;
  }
}
