/**
 * Delivery Notes record adapter — Inventory outbound fulfillment.
 */
import { createGenericRecordAdapter } from './genericRecordAdapter';
import DeliveryNoteLinesRecordSection from '@/components/record-page/sections/DeliveryNoteLinesRecordSection.vue';
import DeliveryNoteWorkflowHeaderActions from '@/components/record-page/sections/DeliveryNoteWorkflowHeaderActions.vue';

const DN_SECTION_STACK = ['details', 'lines', 'related', 'description'];
...
export function createDeliveryNotesRecordAdapter(opts = {}) {
    ...
    getSections(record) {
      ...
      const keys =
        isExpanded && !descriptionFullPage
          ? DN_SECTION_STACK.filter((k) => k === expanded)
          : descriptionFullPage
            ? []
            : DN_SECTION_STACK;
      ...
      const linesSection = {
        key: 'lines',
        title: linesTitle,
        component: DeliveryNoteLinesRecordSection,
        headerActionsComponent: DeliveryNoteWorkflowHeaderActions,
        className: 'pt-0 pb-0',
        actions:
