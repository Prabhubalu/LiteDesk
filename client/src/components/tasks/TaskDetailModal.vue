  ...
  <div class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" @click="$emit('close')">
    <div class="card max-w-3xl w-full max-h-[90vh] overflow-y-auto animate-slide-up" @click.stop>
      <div class="card-header flex items-center justify-between sticky top-0 bg-white dark:bg-gray-800 z-10">
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ t('tasks.taskDetailModalTaskDetails') }}</h2>
      ...
      <div class="card-body space-y-6">
        ...
        <div class="flex items-start justify-between gap-4">
          ...
          <div class="flex gap-2">
            <button @click="$emit('edit', task)" class="p-2 text-gray-600 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-all" :title="t('settings.groupsEditTitle')">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-5 h-5">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
              </svg>
            </button>
            <button @click="confirmDelete" class="p-2 text-gray-600 dark:text-gray-400 hover:text-danger-600 dark:hover:text-danger-400 hover:bg-danger-50 dark:hover:bg-danger-900/20 rounded-lg transition-all" :title="t('settings.modFieldsDelete')">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-5 h-5">
        ...
        <div class="flex items-center gap-3">
          ...
          >
            <option value="todo">{{ t('tasks.taskDetailModalToDo') }}</option>
            <option value="in_progress">{{ t('forms.hubExecutionInProgress') }}</option>
            <option value="waiting">{{ t('tasks.taskDetailModalWaiting') }}</option>
        ...
        <div v-if="task.description">
          <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">{{ t('settings.coreModDetailDescription') }}</h4>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('forms.settingsAssignedTo') }}</p>
          ...
          <div v-if="task.dueDate" class="flex items-center gap-3">
            ...
            <div>
              <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('forms.correctiveDueDate') }}</p>
          ...
          <div v-if="task.createdBy" class="flex items-center gap-3">
            ...
            <div>
              <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('tasks.taskDetailModalCreatedBy') }}</p>
              <p class="text-sm font-medium text-gray-900 dark:text-white">
        ...
        <div v-if="task.estimatedHours" class="flex items-center gap-3">
          ...
          <div>
            <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('tasks.taskDetailModalEstimatedHours') }}</p>
        ...
        <div v-if="task.tags && task.tags.length > 0">
          <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">{{ t('settings.assignRulesCondFieldTags') }}</h4>
