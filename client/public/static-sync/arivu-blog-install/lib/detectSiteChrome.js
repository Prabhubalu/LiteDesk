...
function analyzeLayoutSource(source) {
  ...
  return {
    ...
    components,
    usedNames: components.map((component) => component.name),
  };
}
...
function detectSiteChrome(cwd, project) {
  ...
  const candidateLayouts = layoutFiles
    .filter((layoutPath) => {
      const normalized = layoutPath.split(path.sep).join('/');
      return layoutPath !== rootLayoutPath && !/\/blog\/layout\.(tsx|jsx|js)$/i.test(normalized);
    })
    .map((layoutPath) => {
  ...
  const needsBlogLayoutChrome = !preservesSiteChrome && referenceChromeComponents.length > 0;
  ...
  return {
    ...
    preservesSiteChrome,
    needsBlogLayoutChrome,
    needsHelpLayoutChrome: needsBlogLayoutChrome,
