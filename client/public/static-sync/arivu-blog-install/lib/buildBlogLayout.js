...
const CHROME_WIDTH_OVERRIDE = [
  ...
  '        .arivu-blog-chrome .ld-blog-post,',
...
function buildBlogLayoutContent({ chromeComponents = [] } = {}) {
  ...
  const blogContent = '        <div className="ld-blog-root ld-blog-embed arivu-blog-chrome">{children}</div>';
  ...
  if (container) {
    ...
    middleBlock = blogContent.replace(/^        /, '      ');
  }

  return `import ArivuBlogAssets from './ArivuBlogAssets';
${importLines.join('\n')}
