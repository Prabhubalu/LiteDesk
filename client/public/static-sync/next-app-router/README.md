...
| Path | Purpose |
|------|---------|
| `app/help/[...slug]/page.tsx` | ISR help pages from Arivu export API |
...
1. Copy `app/` into your Next.js project root.
...
5. Deploy. Publish an article in Content Studio — the webhook revalidates affected `/help/*` paths.
...
Or rely on ISR: first visitor to each path triggers export fetch (see `revalidate` in `page.tsx`).
