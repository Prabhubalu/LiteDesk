...
async function downloadInstaller(apiOrigin, cacheDir) {
  const manifestUrl = `${apiOrigin}/static-sync/arivu-blog-install/manifest.json`;
  ...
  for (const relativePath of files) {
    const url = `${apiOrigin}/static-sync/arivu-blog-install/${relativePath}`;
    const response = await fetch(url);
    if (!response.ok) {
      throw new Error(`Failed to download ${url} (${response.status})`);
    }
...
async function main() {
  ...
  const cacheDir = path.join(os.tmpdir(), 'arivu-blog-install');
  await downloadInstaller(apiOrigin, cacheDir);

  const entry = path.join(cacheDir, 'bin/arivu-blog-install.js');
