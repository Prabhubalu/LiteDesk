...
**Alerts:** `INTERNAL_CHAT_MENTIONED` / `INTERNAL_CHAT_MESSAGE_POSTED` → in-app toast + sound (client `alertForInternalChatNotification`); suppress when viewing that space. DMs always notify; channel/record posts notify only when tenant `settings.notifyChannelMessages` is true. SSE plays a debounced chime as fallback when not focused on the space.
