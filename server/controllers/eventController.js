...
exports.createEvent = async (req, res) => {
    try {
        ...
        if (!eventData.eventType || String(eventData.eventType).trim() === '') {
            eventData.eventType = 'Meeting';
        }

        // ===== MEETING CREATE NORMALIZATION =====
        // Map world-class meeting form fields into schema + appointment extension.
        if (!isAuditType && (eventData.eventType === 'Meeting' || !eventData.eventType)) {
            // Attendees: accept [userId] | [{_id}] | [{userId}] → ObjectId strings
            if (Array.isArray(eventData.attendees)) {
                eventData.attendees = eventData.attendees
                    .map((a) => {
                        if (a == null || a === '') return null;
                        if (typeof a === 'object') return a._id || a.userId || a.id || null;
                        return a;
                    })
                    .filter(Boolean);
            }

            // Default duration: 30 minutes when end missing or not after start
            if (eventData.startDateTime) {
                ...
                if (!Number.isNaN(start.getTime())) {
                    ...
                    if (!end || Number.isNaN(end.getTime()) || end <= start) {
                        eventData.endDateTime = new Date(start.getTime() + 30 * 60 * 1000);
                    }
                }
            }

            // Default mode Virtual when unset (remote-first CRM default)
            if (!eventData.meetingMode) {
                eventData.meetingMode = 'Virtual';
            }

            const mode = eventData.meetingMode;
            const ap = (eventData.appointment && typeof eventData.appointment === 'object')
            ...
            } = require('../constants/meetingConferenceProviders');

            if (mode === 'In-person') {
                ap.meetingType = 'offline';
                eventData.conferenceProvider = null;
                eventData.meetingLink = null;
                ap.meetingLink = null;
            } else if (mode === 'Virtual' || mode === 'Hybrid') {
                ap.meetingType = 'online';
                if (!isValidMeetingConferenceProvider(eventData.conferenceProvider)) {
                    eventData.conferenceProvider = 'google_meet';
                }
                if (
                    eventData.meetingLink &&
                    !meetingLinkMatchesProvider(eventData.conferenceProvider, eventData.meetingLink)
                ) {
                    return res.status(400).json({
                        success: false,
                        message: 'Join link does not match the selected conference provider.',
                        code: 'MEETING_LINK_PROVIDER_MISMATCH',
                        field: 'meetingLink'
                    });
                }
                // Pure virtual: no physical site / geo check-in
                if (mode === 'Virtual') {
                    eventData.geoRequired = false;
                    eventData.location = '';
                    if (eventData.geoLocation && typeof eventData.geoLocation === 'object') {
                        eventData.geoLocation = {
                            latitude: null,
                            longitude: null,
                            address: null,
                            radius: eventData.geoLocation.radius ?? 100,
                            accuracy: null
                        };
                    }
                }
            ...
            if (eventData.meetingLink) {
                ap.meetingLink = String(eventData.meetingLink).trim();
            }
            if (Object.keys(ap).length > 0) {
                eventData.appointment = ap;
            }
        }
