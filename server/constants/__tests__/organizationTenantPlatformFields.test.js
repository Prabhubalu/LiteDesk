...
test('isTenantPlatformOrganizationFieldPath matches tenant infrastructure roots and nested paths', () => {
  ...
  assert.equal(isTenantPlatformOrganizationFieldPath('aiSettings'), true);
  assert.equal(isTenantPlatformOrganizationFieldPath('aiSettings.llmProvider'), true);
  assert.equal(isTenantPlatformOrganizationFieldPath('aiSettings.credits.balance'), true);
  assert.equal(isTenantPlatformOrganizationFieldPath('contentPublishing'), true);
  assert.equal(isTenantPlatformOrganizationFieldPath('contentPublishing.headlessApiEnabled'), true);
  assert.equal(isTenantPlatformOrganizationFieldPath('contentPublishing.publishWebhookUrl'), true);
  assert.equal(isTenantPlatformOrganizationFieldPath('gstin'), true);
  assert.equal(isTenantPlatformOrganizationFieldPath('gstRegistrationType'), true);
  assert.equal(isTenantPlatformOrganizationFieldPath('billingAddressStructured'), true);
  assert.equal(isTenantPlatformOrganizationFieldPath('externalReferenceId'), true);
  assert.equal(isTenantPlatformOrganizationFieldPath('syncStatus'), true);
  assert.equal(isTenantPlatformOrganizationFieldPath('lastSyncAt'), true);
...
test('getBaseFieldsForKey(organizations) excludes tenant platform infrastructure paths', () => {
  ...
  assert.equal(keys.some((key) => String(key).startsWith('aiSettings.')), false);
  assert.equal(keys.includes('aiSettings'), false);
  assert.equal(keys.some((key) => String(key).startsWith('contentPublishing.')), false);
  assert.equal(keys.includes('contentPublishing'), false);
  assert.equal(keys.includes('gstin'), false);
  assert.equal(keys.includes('gstRegistrationType'), false);
  assert.equal(keys.includes('stateCode'), false);
  assert.equal(keys.includes('billingAddressStructured'), false);
  assert.equal(keys.includes('externalReferenceId'), false);
  assert.equal(keys.includes('syncStatus'), false);
  assert.equal(keys.includes('lastSyncAt'), false);
