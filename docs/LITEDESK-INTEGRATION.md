...
| `message.bounced` | Hard/soft bounce (Track 3) | Set `status: 'bounced'`, suppress on hard, notify agent |

Future phases will add `message.opened`, `message.clicked`, `message.complained`.
...
## 5. Deliverable 3 — Helpdesk integration
...
- [ ] Implement webhook route with signature verification + raw body
- [ ] Create `amds_webhook_events` collection with unique index on `event_id`
- [ ] Extend ticket reply schema with AMDS fields
- [ ] Wire `sendTicketReplyEmail` into reply creation flow
- [ ] Expose `delivery_status` on ticket API for Vue UI

### Validation tests

- [ ] Send reply → `202` from AMDS → `amds_message_id` stored on reply
- [ ] Email appears in Mailpit within ~5 seconds
- [ ] Webhook received → `delivery_status` becomes `delivered`
...
| `server/services/amds/handlers/helpdesk-event-handler.ts` | Delivery event → ticket update |
| `server/services/helpdesk/send-ticket-reply-email.ts` | Send on reply |
